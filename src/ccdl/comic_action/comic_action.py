...
class proc_img_co:
    ...
    def n21(self, img0):
        ...
        for n in range(self.DIVIDE_NUM * self.DIVIDE_NUM):
            ...
            draw_image(img0, img_copy, src_x, src_y,
                       self.cell_width, self.cell_height, t_x, t_y)
        return img_copy


def downloader(link_info: ComicLinkInfo, driver: webdriver.Chrome):
    try:
        ...
        for i in range(len(pages)):
            ...
            content = requests.get(
                pages[i]['src'], headers=RqHeaders()).content
...
if __name__ == "__main__":
    ...
    downloader(driver, ComicLinkInfo(url))
